...
function layoutRBTree(root) {
    ...
    function traverse(node) {
        ...
        // Записываем связи (ребра) от узла к потомкам
    ...
    function computeWidth(nodeId) {
        if (nodeId === null) return 0; // Теоретически не должно случиться, т.к. никогда не возвращаем null
    ...
    function setPositions(nodeId, depth, leftOffset) {
        ...
        const subtreeW = Math.max(leftW + rightW + HORIZONTAL_GAP, 2 * NODE_RADIUS);
        ...
        if (!node.isNullLeaf) {
            ...
            setPositions(
                node.rightId,
                depth + 1,
                leftOffset + leftW + HORIZONTAL_GAP
            );
        }
    }
